...
void unicore::add_token_to_sale_fund( eosio::name host, eosio::asset amount, eosio::name code ){

    account_index hosts(_me, _me.value);
    auto acc = hosts.find(host.value);
    
    eosio::check(acc != hosts.end(), "host is not found");

    eosio::check(acc -> asset_on_sale.symbol == amount.symbol, "Token symbol mismatch");
    eosio::check(acc -> sale_token_contract == code, "Token contract mismatch");

    hosts.modify(acc, _me, [&](auto &a){
        a.asset_on_sale += amount;
    });

}


[[eosio::action ]]void unicore::enablesale( eosio::name host, eosio::name mode, eosio::name contract, eosio::asset asset_for_sale ){

    account_index hosts(_me, _me.value);
    auto acc = hosts.find(host.value);
    
    eosio::check(acc != hosts.end(), "host is not found");
    eosio::check(mode == "convert"_n, "Only convert mode can be used for now");
    eosio::check(acc -> sale_is_enabled == false, "Sale is already enabled");

    hosts.modify(acc, _me, [&](auto &a){
        a.sale_mode = "convert"_n;
        a.sale_is_enabled = true;
        a.asset_on_sale = asset(0, asset_for_sale.symbol);
        a.asset_on_sale_precision = asset_for_sale.symbol.precision();
        a.asset_on_sale_symbol = asset_for_sale.symbol.code().to_string();
        a.sale_token_contract = contract;
    });
